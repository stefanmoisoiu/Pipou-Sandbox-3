...
public class PlayerAnimator : NetworkBehaviour
{
    ...
    private ushort GetMovementAnim()
    {
        ...
        if (PlayerDive.Diving) return 5;
        if (PlayerWallClimb.WallClimbing) return 6;
        if (PlayerJump.Jumping) return 1;
        if (PlayerCrouch.Crouching) return 3;
        if (PlayerSliding.Sliding || SleighItem.Sleighing) return 4;
        if (PlayerGrounded.IsGrounded) return 0;
        return 2;
    }
